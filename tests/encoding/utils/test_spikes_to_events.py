...
class TestSpikesToEvents(unittest.TestCase):
    ...
    def test_2d_input(self):
        ...
        encoded_signal = np.array([[0, 1, 0], [0, 0, -1]])
        fs = 1000  # 1 kHz
        events = spikes_to_events(encoded_signal, fs)

        expected_times = np.array([0, 1000], dtype=np.uint64)
        expected_x = np.array([0, 0], dtype=np.uint16)
        expected_y = np.array([1, 2], dtype=np.uint16)
        expected_on = np.array([True, False], dtype=bool)
    ...
    def test_3d_input(self):
        ...
        encoded_signal = np.array([[[0, 1], [0, 0]], [[-1, 0], [0, 0]]])
        fs = 1000  # 1 kHz
        events = spikes_to_events(encoded_signal, fs)

        expected_times = np.array([0, 0], dtype=np.uint64)
        expected_x = np.array([0, 1], dtype=np.uint16)
        expected_y = np.array([1, 0], dtype=np.uint16)
        expected_on = np.array([True, False], dtype=bool)
